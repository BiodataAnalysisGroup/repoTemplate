...
fn <- "statisticalAnalysis/output/descriptives.csv"
#Check its existence
if (file.exists(fn)) 
...
# filepath
path <- "statisticalAnalysis/input/Statistics_New.xlsx"
mydata <- read_excel(path)
column_names <- colnames(mydata)

df <- data.frame()
descriptives_path <- 'statisticalAnalysis/output/descriptives.csv'
...
write.csv(data, file = 'statisticalAnalysis/output/Valid_missing_frames.csv')

png(filename = 'statisticalAnalysis/output/boxplots.png', height = 600, width = 1000)
...
write.csv(df, file = 'statisticalAnalysis/output/leveneTest.csv')
...
write.csv(df, file = 'statisticalAnalysis/output/anovaTest.csv')
...
write.csv(robust_matrix, 'statisticalAnalysis/output/robust_tests.csv')
...
write.csv(post_hoc_table, 'statisticalAnalysis/output/Bonferroni_post_hoc_tests.csv')
...
write.csv(df, 'statisticalAnalysis/output/duncan_test.csv')
write.csv(duncan_test$comparison, 'statisticalAnalysis/output/duncan_test_sig.csv')
...
names(df) <- c("Measure")
write.csv(df, 'statisticalAnalysis/output/Kruskal_Wallis_Test_Statistics.csv')

ranks_table <- data.frame(kt2$means$r, kt2$means$rank)
row.names(ranks_table) <- rownames(kt2$means)
names(ranks_table) <- c("N", "Mean Rank")
write.csv(ranks_table, 'statisticalAnalysis/output/Kruskal_Wallis_Test_Mean_Ranks.csv')